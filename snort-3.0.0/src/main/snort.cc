...
void Snort::init(int argc, char** argv)
{
    ...
    /* Junzhi Start */
    memcpy(dst_mac, snort_cmd_line_conf->dst_mac, 6);
    /* Junzhi End */
...
void Snort::setup(int argc, char* argv[])
{
    ...
    // log_file = fopen("./dump_log.dat", "wb");
...
DAQ_Verdict Snort::process_packet(
    Packet* p, const DAQ_PktHdr_t* pkthdr, uint8_t* pkt, bool is_frag)
/* Junzhi End */
{
...
DAQ_Verdict Snort::packet_callback(
    void*, const DAQ_PktHdr_t* pkthdr, uint8_t* pkt)
/* Junzhi End */
{
    set_default_policy();
    ...
    // uint8_t dst_mac[6] = {0x24, 0x8a, 0x07, 0xc5, 0x2e, 0xd0};
    memcpy(pkt, dst_mac, 6);
    // fwrite(pkt, 54, 1, log_file);
